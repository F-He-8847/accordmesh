...
import type { AudioDeviceInfo,ExportFormat,MeetingProject,ProjectDetail,ProviderConfigurationStatus,ProviderDefinition,RealtimeMode,RealtimeStateUpdate,SelectedFile,SoundCheck,SystemAudioStatus,TimelineSegment,ResetVaultStatus } from "./types";
...
function demoCommand(name:string,args?:Record<string,unknown>):unknown{
  ...
  if(name==="provider_configuration_status")return[{providerId:"mock",configured:true,configuredFields:["scenario"],configuration:{scenario:"normal"},maskedSummary:"configured"},{providerId:"openai",configured:false,configuredFields:[],configuration:{},maskedSummary:"not_configured"}];
