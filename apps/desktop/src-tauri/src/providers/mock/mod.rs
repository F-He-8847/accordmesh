...
impl ComparisonReportProvider for MockProvider {
    async fn compare(
        ...
        let both = realtime
            ...
            .cloned()
            .collect::<Vec<_>>();
        Ok(self.draft("intelligent_comparison_report","mock-comparison-v1",json!({"overallAssessment":"The recording confirms that the pilot target is conditional and that no final public launch date was committed.","correctlyCaptured":[{"type":"Condition","status":"confirmed","text":"The May 12 pilot target depends on security sign-off by the previous Friday.","evidenceRefs":both}],"missedOrIncomplete":[{"type":"UnresolvedIssue","status":"incomplete_in_realtime","text":"Legal review timing still needs confirmation.","evidenceRefs":uploaded}],"correctedInterpretations":[{"type":"Decision","status":"refined","text":"May 12 is a conditional pilot target, not a public launch commitment.","evidenceRefs":both}],"newlyDiscovered":[{"type":"FollowUp","status":"new_detail","text":"Translation review effort must also be confirmed.","evidenceRefs":uploaded}],"guidanceRevisions":[{"type":"Guidance","status":"guidance_changed","text":"Confirm the security sign-off deadline, legal review timing, and translation review effort.","evidenceRefs":both}],"conclusionChanges":[],"recommendedFollowUps":["Confirm the security sign-off deadline.","Confirm legal review timing and translation review effort."]})))
    }
}
